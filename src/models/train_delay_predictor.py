"""
Bus Delay Prediction using Artificial Neural Networks (ANN) - V1 Baseline
==========================================================================

This script implements delay prediction using academically recognized ANN models:
1. Multi-Layer Perceptron (MLP) - Baseline
2. LSTM (Long Short-Term Memory)
3. GRU (Gated Recurrent Unit)

Data Split: Temporal - Train on <2025, Test on >=2025
...
EXPERIMENT_VERSION = "v1_baseline_temporal"


# =============================================================================
# Data Processing with Temporal Split
# =============================================================================

def load_and_preprocess_data_temporal(sample_size: int = 500000):
    """
    Load and preprocess arrival/departure data for delay prediction.

    TEMPORAL SPLIT: Train on <2025, Test on >=2025
    """
    print("\n" + "="*60)
    print("Loading and Preprocessing Data (V1 - Temporal Split)")
    print("="*60)
    ...
    # Filter outliers (delay between -30 and 60 minutes)
    ...
    # Encode categorical features - fit on combined to handle unseen categories
    ...
    # Scheduled headway (fill missing with training median)
...
class MLPPredictor(nn.Module):
    """Multi-Layer Perceptron for delay prediction."""
...
class LSTMPredictor(nn.Module):
    """LSTM-based delay predictor. Reference: Hochreiter & Schmidhuber (1997)"""
...
class GRUPredictor(nn.Module):
    """GRU-based delay predictor. Reference: Cho et al. (2014)"""
...
def evaluate_model(model, test_loader, scaler_y, model_name: str = "model"):
    ...
    print(f"\n{model_name} (V1 Temporal) Evaluation Results:")
    print(f"  MSE:  {mse:.4f}")
...
def generate_report(all_metrics, results, feature_columns, save_path):
    ...
    report = f"""# Bus Delay Prediction - V1 Baseline (Temporal Split)
...
def main():
    ...
    (X_train, y_train, X_test, y_test,
     feature_columns, scaler_X, scaler_y, y_train_orig, y_test_orig) = load_and_preprocess_data_temporal()
    ...
    metrics_df = pd.DataFrame(all_metrics)
    print("\n" + "="*60)
    print("V1 Baseline Temporal Split Results Summary")
    print("="*60)
    ...
    for ax, (model_name, data) in zip(axes, results.items()):
        ...
        ax.set_title(f'{model_name} (V1 Temporal) Training')
        ax.legend()
    ...
    print("\n" + "="*60)
    print("Experiment V1 (Temporal Split) Complete!")
    print("="*60)
